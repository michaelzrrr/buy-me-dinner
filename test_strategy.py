...
def plot_history(trader: MyStrategy):
...
def run_streaming_data(data_frame, trader: MyStrategy, title, tk = False):
...
def run_all_data_stream():
    ...
    for obj in s3.Bucket(bucket_name).objects.all():
        object_key = obj.key
        csv_obj = s3.Bucket(bucket_name).Object(object_key).get()
        data = pd.read_csv(csv_obj['Body'])

        trader = MyStrategy(5000, 116.88, 1000)
        run_streaming_data(data, trader, object_key)

def run_single_data_stream(filename):
    s3 = boto3.resource(
        service_name='s3',
        region_name='us-east-2',
        aws_access_key_id=secret_keys.access_key,
    ...
    csv_obj = s3.Bucket(bucket_name).Object(filename).get()
    data = pd.read_csv(csv_obj['Body'])

    trader = MyStrategy(5000, 117.345, 1000)
    run_streaming_data(data, trader, filename)


def tk_run_single_data_stream(filename):
    s3 = boto3.resource(
        service_name='s3',
        region_name='us-east-2',
        aws_access_key_id=secret_keys.access_key,
    ...
    trader = MyStrategy(5000, 117.345, 1000)
    return run_streaming_data(data, trader, filename, True)
